...
class LoginGUI:
    def __init__(self, root, on_login_success):
        ...
        self.on_login_success = on_login_success  # פונקציה שתופעל לאחר התחברות מוצלחת
    ...
    def login(self):
        """ מתבצע כאשר המשתמש לוחץ על כפתור ההתחברות """
        username = self.username_entry.get()
        password = self.password_entry.get()
        role = self.login_manager.authenticate_user(username, password)

        if role:
            self.root.destroy()  # סגירת מסך ההתחברות
            self.on_login_success(username, role)  # קריאה למסך הבא
        else:
